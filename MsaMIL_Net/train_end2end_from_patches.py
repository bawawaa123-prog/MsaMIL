...
_DEFAULT_DGMSAMIL_PYTHON = os.environ.get("DGMSAMIL_PYTHON", sys.executable)


def _maybe_reexec_into_dgmsamil() -> None:
	"""Best-effort re-exec into the DgMsa-MIL conda environment Python.

	This avoids common failures when the user runs `python ...` from a shell
	that is not in the intended env (e.g., missing torch).

	Control via env vars:
	- DGMSAMIL_AUTO_SWITCH=0 to disable
	- DGMSAMIL_PYTHON=/path/to/python to override target
	"""
	if os.environ.get("DGMSAMIL_AUTO_SWITCH", "1") == "0":
		return
	if os.environ.get("_DGMSAMIL_REEXECED", "0") == "1":
		return
	target = Path(os.environ.get("DGMSAMIL_PYTHON", _DEFAULT_DGMSAMIL_PYTHON)).expanduser()
	try:
	...
	if target.exists() and cur != target:
		os.environ["_DGMSAMIL_REEXECED"] = "1"
		print(f"[Env] Re-exec into DgMsa-MIL python: {target}")
...
try:
	...
	# If torch is missing, try to re-exec into the configured python.
	_maybe_reexec_into_dgmsamil()
	raise

# If DGMSAMIL_PYTHON is set and differs from current, re-exec.
_maybe_reexec_into_dgmsamil()
...
class PhikonDistillWSIDataset(Dataset):
	...
	def _align_student_patches_to_teacher(
		...
		*,
		...
		if len(self.scales) == 1 and expected_n > 0:
			only_scale = self.scales[0]
			want_code = int(SCALE_ENCODING.get(only_scale.name, 0))
			try:
				uniq = np.unique(teacher_scales.astype(np.int64))
			except Exception:
				uniq = np.array([], dtype=np.int64)
			if uniq.size == 1 and int(uniq[0]) == want_code:
				if not getattr(self, "_warned_single_scale_order_align", False):
					print(
						"[DistillDataset] INFO single-scale: using order-based teacher/patch alignment "
						"(teacher coords may be normalized with multi-scale extents)."
					)
					setattr(self, "_warned_single_scale_order_align", True)
				paths_all = [r.path for r in records]
				take_n = min(expected_n, len(paths_all))
				if len(paths_all) > take_n:
					readable: List[Path] = []
					for p in paths_all:
						if self._is_png_readable(p):
							readable.append(p)
							if len(readable) >= take_n:
								break
						if len(readable) == take_n:
							paths_all = readable
				paths_all = paths_all[:take_n]
				teacher_idx = np.arange(take_n, dtype=np.int64)
				out = (paths_all, teacher_idx)
...
def _effective_patch_batch_size(*, cfg: Any, epoch: int, is_train: bool) -> int:
	"""Choose NMFEM micro-batch size (chunk size) for the current epoch.
...
def encode_patches_in_chunks(
	*,
	...
	- When NMFEM is trainable, autograd retains activations for every chunk until
	  backward, so chunking alone may still OOM.
	- use_checkpoint=True trades extra compute for significantly lower VRAM by
	  recomputing NMFEM activations during backward.
	"""
	...
	NMFEM_trainable = any(p.requires_grad for p in NMFEM.parameters())
	...
	for i in range(0, int(patches.shape[0]), chunk):
		...
		if use_checkpoint:
			...
			if NMFEM_trainable and (not getattr(out, "requires_grad", False)):
				if not getattr(encode_patches_in_chunks, "_warned_ckpt_no_grad", False):
					print("[WARN] checkpoint_NMFEM_chunks produced non-grad output; falling back to normal forward for stability.")
					setattr(encode_patches_in_chunks, "_warned_ckpt_no_grad", True)
				out = NMFEM(micro)
			outs.append(out)
		else:
			outs.append(NMFEM(micro))
...
class TrainConfig:
	...
	# Which scales to use for training/eval. Example for single-scale: ("20x",)
	scale_names: Tuple[str, ...] = ("20x", "10x", "5x")

	input_size: int = 256
	bag_size: int = 256
	patch_batch_size: int = 16
	# Optional: reduce NMFEM micro-batch once IAAM fine-tuning starts (OOM mitigation).
	# 0 disables.
	...
	# Optional: override NMFEM micro-batch for validation only (0 => use effective train value).
	...
	# Memory: checkpoint NMFEM chunk forwards (recompute on backward) to reduce VRAM.
	checkpoint_NMFEM_chunks: bool = True
	...
	# LR split for NMFEM parts
	nmfem_backbone_lr_mult: float = 0.2
	...
	save_dir: str = "results/nmfem_distill_phikon"


# Config to edit for your run.
CFG = TrainConfig(
	teacher_features_dir="data/features_phikon_Yi_20x",
	split_csv="splits/YiYuan/splits_phikon_02.csv",
	fold=0,
	sort_order="xy",
	scale_names=("20x",),
	patches_20x="data/patches_20x",
	patches_10x="data/patches_10x",
	patches_5x="data/patches_5x",
	iaam_ckpt="auto",
	input_size=512,
	bag_size=512,
	val_bag_size=0,
	...
	iaam_lr_mult=0.05,
	# Late-stage CE-only fine-tune: smaller LR, but slightly larger IAAM LR to adapt to NMFEM's distribution.
	...
	grad_clip_norm=1.0,
	nmfem_backbone_lr_mult=0.2,
	num_workers=4,
	deterministic_eval=True,
	skip_missing=True,
	save_dir="results/nmfem_distill_phikon_q10_20x",
	debug=False,
...
def main() -> None:
	...
	all_scales = {
		"20x": ScaleConfig(name="20x", patch_size=512, root=Path(cfg.patches_20x).expanduser().resolve()),
		"10x": ScaleConfig(name="10x", patch_size=1024, root=Path(cfg.patches_10x).expanduser().resolve()),
		"5x": ScaleConfig(name="5x", patch_size=2048, root=Path(cfg.patches_5x).expanduser().resolve()),
	}
	selected = tuple(str(s).strip() for s in getattr(cfg, "scale_names", ("20x", "10x", "5x")) if str(s).strip())
	bad = [s for s in selected if s not in all_scales]
	if bad:
		raise ValueError(f"Unknown scale_names={bad}. Allowed: {list(all_scales.keys())}")
	if len(selected) == 0:
		raise ValueError("scale_names cannot be empty")
	scales = [all_scales[s] for s in selected]
	...
	NMFEM = NMFEM(
		output_dim=1024,
		num_heads=cfg.NMFEM_heads,
		num_layers=cfg.NMFEM_layers,
		pretrained=True,
	...
	def _apply_backbone_unfreeze(unfreeze_blocks: int) -> None:
		...
		if not bool(cfg.freeze_backbone):
			...
			for p in NMFEM.backbone.parameters():
		...
		if n <= 0:
			return
		blocks = list(NMFEM.backbone.children()) if isinstance(NMFEM.backbone, torch.nn.Sequential) else []
		if not blocks:
			return
		for m in blocks[-n:]:
	...
	# Ensure the initial freeze/unfreeze policy matches cfg even if NMFEM init logic changes.
	_apply_backbone_unfreeze(int(cfg.unfreeze_backbone_blocks))
	if debug:
		tr, tot = _count_trainable_params(NMFEM)
		try:
			tr_bb, tot_bb = _count_trainable_params(NMFEM.backbone)
		except Exception:
			tr_bb, tot_bb = -1, -1
		print(f"[Debug] NMFEM trainable={tr}/{tot} | backbone trainable={tr_bb}/{tot_bb}")
		if cfg.freeze_backbone:
			print(f"[Debug] freeze_backbone=True, unfreeze_backbone_blocks={cfg.unfreeze_backbone_blocks}")

	# Optimizer: split NMFEM backbone vs head/transformer for more stable fine-tuning.
	# IMPORTANT: include ALL backbone params so later unfreezing works without rebuilding the optimizer.
	backbone_params = list(NMFEM.backbone.parameters())
	head_params: List[torch.nn.Parameter] = []
	for name, p in NMFEM.named_parameters():
	...
	param_groups = [
		...
		},
		{
			...
			"lr": float(cfg.lr) * float(cfg.NMFEM_backbone_lr_mult),
			"weight_decay": float(cfg.weight_decay),
		},
		{
			"name": "iaam",
	...
	if cfg.resume_path:
		...
		if not isinstance(resume_payload, dict) or "NMFEM" not in resume_payload:
			raise ValueError("Resume checkpoint must be a dict containing at least key 'NMFEM'.")
		NMFEM.load_state_dict(resume_payload["NMFEM"], strict=True)
	...
	best_path = run_dir / "best_NMFEM.pth"
	last_path = run_dir / "last_NMFEM.pth"
	...
	if resumed_epoch is not None:
		...
		if do_val:
			...
			if isinstance(auc, (int, float)) and not (isinstance(auc, float) and math.isnan(auc)) and float(auc) > best_auc:
				...
				print(f"[Best] Updated best_NMFEM.pth from resume-val (val_auc={best_auc:.4f})")
		else:
			print(
	...
	NMFEM_tr, NMFEM_tot = _count_trainable_params(NMFEM)
	...
	if iaam_group is not None:
		...
		print(
			...
			f"| NMFEM={NMFEM_tr}/{NMFEM_tot} trainable | iaam_group params={len(gp)} req_grad={req} lr={float(iaam_group.get('lr', 0.0)):.2e}"
		)
	else:
		print(
			f"[Debug] Pre-train trainability: IAAM={iaam_tr}/{iaam_tot} trainable (mode={'train' if iaam.training else 'eval'}) "
			f"| NMFEM={NMFEM_tr}/{NMFEM_tot} trainable | iaam_group=NOT_FOUND"
		)

	def _backbone_trainable() -> bool:
		try:
			return any(p.requires_grad for p in NMFEM.backbone.parameters())
		except Exception:
			return any(p.requires_grad for p in NMFEM.parameters())
	...
	def _apply_epoch_lrs(epoch_num: int) -> None:
		...
		lr_head = base_lr
		lr_bb = base_lr * float(cfg.NMFEM_backbone_lr_mult)
		lr_iaam = base_lr * iaam_mult
		for g in optimizer.param_groups:
			name = str(g.get("name", ""))
			if name == "NMFEM_head":
				g["lr"] = float(lr_head)
			elif name == "NMFEM_backbone":
				g["lr"] = float(lr_bb)
			elif name == "iaam":
				g["lr"] = float(lr_iaam)
	...
	for epoch in range(start_epoch, cfg.epochs + 1):
		...
		if (
			...
			if bool(cfg.debug):
				...
				print(
					...
					f"| NMFEM trainable={tr}/{tot} backbone={tr_bb}/{tot_bb}"
				)
		...
		if freeze_bn:
			_set_batchnorm_eval(NMFEM.backbone)
			if bool(cfg.debug):
				print(
		...
		NMFEM_any_trainable = any(p.requires_grad for p in NMFEM.parameters())
		...
		for step_idx, batch in enumerate(pbar, start=1):
			...
			use_NMFEM_ckpt = bool(getattr(cfg, "checkpoint_NMFEM_chunks", False)) and NMFEM_any_trainable
			with torch.autocast(device_type=str(device.type), enabled=use_amp):
				try:
					student_feats = encode_patches_in_chunks(
						NMFEM=NMFEM,
					...
					print(
						...
						f"chunk_size={int(chunk_size)} checkpoint_NMFEM={bool(use_NMFEM_ckpt)}"
					)
					optimizer.zero_grad(set_to_none=True)
					if device.type == "cuda":
						try:
			...
			if float(getattr(cfg, "grad_clip_norm", 0.0)) and float(cfg.grad_clip_norm) > 0.0:
				...
				# If IAAM is trainable, clip both NMFEM+IAAM to avoid occasional spikes.
				if bool(cfg.iaam_train_start_epoch >= 1 and epoch >= int(cfg.iaam_train_start_epoch)):
					params = list(NMFEM.parameters()) + list(iaam.parameters())
				else:
					params = list(NMFEM.parameters())
		...
		if isinstance(auc, (int, float)) and not (isinstance(auc, float) and math.isnan(auc)) and float(auc) > best_auc:
			...
			print(f"[Best] Updated best_NMFEM.pth (val_auc={best_auc:.4f})")
