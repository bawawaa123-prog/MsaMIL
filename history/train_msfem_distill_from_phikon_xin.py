...
class PhikonDistillWSIDataset(Dataset):
	...
	def _align_student_patches_to_teacher(
		...
		*,
		...
		# Only count teacher entries for the selected scales.
		expected_n = int(len(allowed_teacher_indices))
		if len(aligned_paths) == 0 or (expected_n > 0 and len(aligned_paths) < max(1, int(0.6 * min(expected_n, len(records))))):
			...
			# Keep mapping in terms of ORIGINAL teacher indices for selected scales.
			if len(paths_all) >= expected_n:
				paths_all = paths_all[:expected_n]
				teacher_idx = np.asarray(allowed_teacher_indices[:expected_n], dtype=np.int64)
			else:
				teacher_idx = np.asarray(allowed_teacher_indices[: len(paths_all)], dtype=np.int64)
...
@dataclass
class TrainConfig:
	"""训练配置（全部在代码内写死，便于复现实验）。

	字段说明（按出现顺序）：
	- teacher_features_dir: 教师特征目录（Phikon 离线特征），应包含每个 WSI 的特征文件。
	- split_csv: 划分文件路径（包含 wsi_id/label/split/fold 等列）。
	- fold: 使用哪一个 fold（与 split_csv 的 fold 列匹配）。
	- sort_order: patch 排序方式（"xy" 或 "yx"）；必须与特征提取脚本的排序一致，否则蒸馏对齐会错位。
	- scale_names: 本次训练使用哪些倍率（例如 ("20x",) 做单尺度消融；默认三尺度）。
	- patches_20x/patches_10x/patches_5x: 各倍率 patch 的根目录（目录下应按 wsi_id/xxx.png 组织）。
	- iaam_ckpt: IAAM 聚合器的 checkpoint 路径（也可传 "auto" 自动寻找 results/**/best_model_*.pth）。

	数据与显存：
	- input_size: patch 输入分辨率（会 resize 到该大小；建议可被 32 整除，符合 EfficientNet/特征提取对齐习惯）。
	- bag_size: 每个 WSI 采样的 patch 数（训练用）；越大越稳定但越慢。
	- patch_batch_size: MSFEM 编码 patch 的 micro-batch（每次前向多少张 patch），直接影响显存峰值。
	- patch_batch_size_after_iaam_train: IAAM 解冻训练后可改用更小 micro-batch 以缓解 OOM（0 表示不启用）。
	- freeze_backbone_bn_patch_batch_leq: 当 micro-batch <= 该阈值时冻结 backbone 的 BN 统计（0 表示不启用）。
	- val_patch_batch_size: 仅验证阶段的 micro-batch（0 表示与训练一致）。

	MSFEM 结构与可训练性：
	- msfem_layers/msfem_heads: MSFEM 内部 Transformer 层数/多头数。
	- freeze_backbone: 是否冻结 EfficientNet backbone。
	- unfreeze_backbone_blocks: 解冻 backbone 的最后 N 个 block（当 freeze_backbone=False 或逐步解冻启用时生效）。
	- unfreeze_backbone_blocks_after_warmup: warmup 后进一步解冻的 block 数（0 表示不启用逐步解冻）。
	- checkpoint_iaam: 是否对 IAAM 使用 gradient checkpoint（省显存但更慢）。
	- checkpoint_msfem_chunks: 是否对 MSFEM 的 chunk 编码使用 checkpoint（省显存但更慢）。

	IAAM 端到端微调：
	- iaam_train_start_epoch: IAAM 从第几个 epoch 开始解冻训练（-1 表示全程冻结；0 表示自动=warmup+1）。
	- iaam_lr_mult: IAAM 学习率倍率（相对 lr）。
	- disable_logit_kd_when_training_iaam: 当 IAAM 可训练时，是否关闭 logit KD（避免“老师 logits 也在变”导致不稳定）。

	训练时长与优化器：
	- epochs/warmup_epochs: 总 epoch / warmup epoch（warmup 期间通常只做特征蒸馏，降低不稳定）。
	- lr/weight_decay: 主学习率/权重衰减。
	- amp: 是否开启训练 AMP（混合精度）。
	- val_amp: 验证是否使用 AMP（默认 False 用 fp32 更稳）。
	- seed: 随机种子。

	损失权重与调度：
	- lambda_feat: 特征蒸馏权重（student feat vs teacher feat）。
	- lambda_feat_decay_start_epoch/lambda_feat_decay_epochs/lambda_feat_final: 特征蒸馏晚期衰减窗口与最终值（用于切到 CE-only）。
	- lambda_kd: logit KD 权重。
	- lambda_ce: 分类交叉熵权重。
	- lambda_ce_final: 晚期 CE boost 的最大权重。
	- lambda_ce_boost_start_epoch/lambda_ce_boost_epochs: CE boost 的起止窗口（0 表示自动与 feat 衰减同步）。
	- temperature: KD 温度系数。
	
	稳定性/阶段切换：
	- ce_start_epoch/kd_start_epoch: CE/KD 从第几个 epoch 开始参与（0 表示自动=warmup+1）。
	- ramp_epochs: 从 0 线性 ramp 到目标权重的 epoch 数（避免“硬切换”梯度爆炸）。
	- lr_after_warmup_mult: warmup 结束后学习率倍率（缓解阶段切换）。
	- lr_after_distill_mult/lr_after_distill_start_epoch: 最终 CE-only 阶段进一步降 LR 的倍率与起始 epoch。
	- grad_clip_norm: 梯度裁剪阈值（0 表示不裁剪）。

	运行控制与鲁棒性：
	- debug: 是否打印更多调试信息。
	- skip_nonfinite_batches/max_nonfinite_skips_per_epoch: 遇到 NaN/Inf 是否跳过该 batch，以及每个 epoch 最多允许跳过多少（0 表示不限）。
	- skip_oom_batches/max_oom_skips_per_epoch: 遇到 CUDA OOM 是否跳过该 batch，以及每个 epoch 最多允许跳过多少（0 表示不限）。
	- empty_cache_each_epoch: 每个 epoch 结束是否清理 CUDA cache（减少碎片化，可能稍慢）。
	- resume_path: 从哪个 checkpoint 恢复（None 表示不恢复）。
	- resume_val_policy: 恢复时是否对“加载的那个 epoch”补跑一次验证（auto/always/never）。
	- save_every_epoch: 是否每个 epoch 都保存一次 checkpoint。
	
	验证与加载：
	- val_bag_size: 验证阶段 bag_size（0 表示与训练一致）。
	- msfem_backbone_lr_mult: backbone 学习率倍率（相对 lr，通常更小）。
	- iaam_lr_mult_after_distill/iaam_lr_mult_after_distill_start_epoch: 最终阶段 IAAM 使用单独 LR 倍率与起始 epoch（0 表示不启用）。
	- num_workers: DataLoader 并行 worker 数。
	- deterministic_eval: 验证是否固定采样（True 更可复现）。
	- skip_missing: 遇到缺失 patch/特征文件时是否跳过样本（True 更鲁棒）。
	- save_dir: 本次运行输出目录（会写 checkpoints、metrics.jsonl 等；建议每次实验改名避免覆盖）。
	"""
	teacher_features_dir: str
	split_csv: str
	fold: int
	...
	# Which scales to use for training/eval.
	# Default keeps the original multi-scale behavior.
	# Example for single-scale ablation: ("20x",)
...
CFG = TrainConfig(
	...
	iaam_ckpt="/private/ljh-data/shared/MsaMIL/MsaMIL_Net/results/YiYuan/features_phikon_queries_06/best_model_20260119_151110.pth",
	...
	skip_missing=True,
	save_dir="results/msfem_distill_phikon_q06",
	debug=False,
